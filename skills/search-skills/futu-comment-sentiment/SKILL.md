...
```bash
( if command -v futu-skills >/dev/null 2>&1; then futu-skills check --quiet 2>/dev/null | grep '^futu-comment-sentiment'; else echo '__CLI_MISSING__'; fi ) > /tmp/.futu-comment-sentiment-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.futu-comment-sentiment-update 2>/dev/null; rm -f /tmp/.futu-comment-sentiment-update
```
...
- **Output starting with `futu-comment-sentiment`** (this skill has an update available): append a notice at the **end** of your reply:
  > `futu-comment-sentiment` has a new version available. If you agree, I can upgrade it for you right away.

  If the user agrees, run `futu-skills upgrade futu-comment-sentiment` to perform the upgrade.

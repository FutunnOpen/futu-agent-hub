...
```bash
futu-skills check --quiet 2>/dev/null | grep '^futu-comment-sentiment' > /tmp/.futu-comment-sentiment-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.futu-comment-sentiment-update 2>/dev/null; rm -f /tmp/.futu-comment-sentiment-update
```

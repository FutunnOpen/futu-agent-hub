...
```bash
futu-skills check --quiet 2>/dev/null | grep '^futu-stock-digest' > /tmp/.futu-stock-digest-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.futu-stock-digest-update 2>/dev/null; rm -f /tmp/.futu-stock-digest-update
```

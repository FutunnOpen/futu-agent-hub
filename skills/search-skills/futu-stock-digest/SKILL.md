...
description: >-
  Interprets the latest news for ONE user-specified stock: extracts key events,
  summarizes impact direction (bullish/bearish/neutral) — analysis, not a raw list.
  TRIGGER: stock + interpretation verb (解读/分析/研判/总结/摘要/interpret/analyze/
  summarize). SKIP: bare news-intent without interpretation verb like "最新新闻/最新消息"
  (→ futu-news-search), community sentiment/情绪 (→ futu-comment-sentiment).
metadata:
  version: 0.0.4
  author: Futu
  requires:
...
```bash
( if command -v futu-skills >/dev/null 2>&1; then futu-skills check --quiet --slug futu-stock-digest 2>/dev/null; else echo '__CLI_MISSING__'; fi ) > /tmp/.futu-stock-digest-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.futu-stock-digest-update 2>/dev/null; rm -f /tmp/.futu-stock-digest-update
```

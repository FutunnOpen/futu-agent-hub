...
```bash
futu-skills check --quiet 2>/dev/null | grep '^futu-news-search' > /tmp/.futu-news-search-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.futu-news-search-update 2>/dev/null; rm -f /tmp/.futu-news-search-update
```

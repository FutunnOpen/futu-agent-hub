...
```bash
( if command -v futu-skills >/dev/null 2>&1; then futu-skills check --quiet --slug futu-news-search 2>/dev/null; else echo '__CLI_MISSING__'; fi ) > /tmp/.futu-news-search-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.futu-news-search-update 2>/dev/null; rm -f /tmp/.futu-news-search-update
```

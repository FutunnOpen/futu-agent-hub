...
```bash
futu-skills check --quiet 2>/dev/null | grep '^futu-technical-anomaly' > /tmp/.futu-technical-anomaly-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.futu-technical-anomaly-update 2>/dev/null; rm -f /tmp/.futu-technical-anomaly-update
```
...
This skill depends on Futu **OpenD** (default `127.0.0.1:11111`) to fetch technical anomaly data. **Before** running the Workflow below, you must confirm OpenD is installed and running, otherwise `scripts/handle_technical_anomaly.py` will fail because it cannot connect to OpenD.
...
> **Hard rule**: When OpenD detection fails, or OpenD is not installed or not running, **do not execute `scripts/handle_technical_anomaly.py` directly**. You must first complete the pre-flight checks above and ensure OpenD is ready before proceeding.

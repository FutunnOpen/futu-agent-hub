...
```bash
( if command -v futu-skills >/dev/null 2>&1; then futu-skills check --quiet --slug futu-derivatives-anomaly 2>/dev/null; else echo '__CLI_MISSING__'; fi ) > /tmp/.futu-derivatives-anomaly-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.futu-derivatives-anomaly-update 2>/dev/null; rm -f /tmp/.futu-derivatives-anomaly-update
```

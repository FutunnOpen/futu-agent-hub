...
```bash
( if command -v futu-skills >/dev/null 2>&1; then futu-skills check --quiet 2>/dev/null | grep '^futu-capital-anomaly'; else echo '__CLI_MISSING__'; fi ) > /tmp/.futu-capital-anomaly-update 2>/dev/null & curl -sG '...' ...; wait; cat /tmp/.futu-capital-anomaly-update 2>/dev/null; rm -f /tmp/.futu-capital-anomaly-update
```
...
- **Output starting with `futu-capital-anomaly`** (this skill has an update available): append a notice at the **end** of your reply:
  > `futu-capital-anomaly` has a new version available. If you agree, I can upgrade it for you right away.

  If the user agrees, run `futu-skills upgrade futu-capital-anomaly` to perform the upgrade.

...
将版本检测**内联**到 skill 的**第一次 Bash 调用**（即下文 "自动检测操作系统" 这一步）中。**不要为版本检测单独发起一次工具调用**。同一次内联检测同时完成三件事：
...
```bash
( if command -v futu-skills >/dev/null 2>&1; then futu-skills check --quiet 2>/dev/null | grep -E '^(cli|install-futu-opend)'; else echo '__CLI_MISSING__'; fi ) > /tmp/.install-futu-opend-update 2>/dev/null & uname -s 2>/dev/null || echo Windows; wait; cat /tmp/.install-futu-opend-update 2>/dev/null; rm -f /tmp/.install-futu-opend-update
```
...
- **输出包含以 `install-futu-opend` 开头的行**（本 skill 有新版本）：在回复**末尾**附加一句提示：
  > `install-futu-opend` 有新版本可用，如果你同意，我可以立即帮你升级。

  用户同意后执行 `futu-skills upgrade install-futu-opend` 完成升级。（该命令会顺带升级 `futu-skills` 管理器自身，因此 skill 有更新时无需再单独提示 CLI 升级。）
- **输出仅包含以 `cli` 开头的行**（本 skill 已是最新，但 `futu-skills` 管理器自身有新版本）：在回复**末尾**附加一句提示：
  ...
  若同时出现 `cli` 行和 `install-futu-opend` 行，仅展示上面的 skill 升级提示——`futu-skills upgrade install-futu-opend` 会顺带刷新管理器。
